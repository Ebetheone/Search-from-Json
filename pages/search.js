...
function Search({ data }) {
  ...
  const { byRegion, byVariety, byName } = router.query;
  let needData;
  if (byRegion) {
    needData = data.filter(
      (wine) => wine.region_1 == byRegion || wine.region_2 == byRegion
    );
  } else if (byVariety) {
    needData = data.filter((wine) => wine.variety.toLowerCase().includes(byVariety));
  } else {
    needData = data.filter((wine) => wine.title.toLowerCase().includes(byName));
  }
  const [value, setValue] = useState();
  const [secondValue, setSecondValue] = useState();
  const [initialData, setInitialData] = useState(needData);
  ...
  const filterData = useCallback(() => {
    ...
    setInitialData(datt);
  }, [firstPrice, needData, price, secondValue, value]);
  const columns = [{
    label: 'Title',
    dataField: 'title',
	width: 200
